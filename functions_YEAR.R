...
pull_fishery_info <- function(this_run, ecocap = T, ssb = T){
  ...
  if(ecocap){
    ...
    # handle time step
    # this has 0-109 in days
    # as it talks about expected catch the following year and it produces scalars for mfc the coming year, this is one case where we stagger up
    ecocap_report <- ecocap_report %>%
  ...
  biom_filtered <- biom_long %>%
    ...
    filter(Time < max(Time)) %>% # 3/6 ALBI: NB!!! This works only because, using our logic, the very last data point for biomass refers to year yrmax + 1
  ...
  # it uses max biomass the year prior to account for recruitment / aging dynamics
  ...
  biom_selex_max <- biom_filtered %>%
    ...
    ungroup() %>%
    mutate(Year = Year + 1) %>% # 3/6 New approach: stop using Time, convert everything to year for ease of interpreations
    select(-Time)
  ...
  # drop T = 0 (initialization, no catch)
  # the terminal value is immportant, unlike the one from biomass, because it describes catch for the last simulation year
  catch_long <- catch_long %>%
    filter(Time > 0) %>%
    mutate(Year = ceiling(Time/365)) %>% # catch at Year = 1 referes to catch during the 1st year, and so on
    select(-Time)
  ...
  for(sp in oy_species) {
    ...
    sp_data <- biom_selex_max %>%
      filter(Code == sp) %>%
      left_join(filter(catch_long, Code == sp), by = c("Year", "Code")) %>% # ALBI 3/6 join by year, no longer doing lags
      left_join(filter(biom_tot_all, Code == sp), by = c("Year", "Code")) %>%
      mutate(
  ...
  if(ecocap){
    ...
    ecocap_report <- ecocap_report %>%
      select(Year, SpeciesName, FisheryName, SpBased_Frescale, PostSystCap_Frescale) %>%
      filter(SpeciesName %in% oy_species,
             FisheryName %in% oy_fleets) %>%
      mutate(oy_rescale = PostSystCap_Frescale / SpBased_Frescale) %>%
      select(Year, SpeciesName, oy_rescale) %>%
      rename(Code = SpeciesName)
    
    # Final join
    res_df <- res_df %>%
      left_join(ecocap_report, by = c("Year", "Code"))
    
  }
  
  # when the HCR closes a fishery, tings get funky in the ecocap output file
  # basically, if catch is 0 then the OY rescaling should be NA / NaN (there is no catch to rescale), so let's put a guard for that here
  res_df <- res_df %>%
    rowwise() %>%
...
plot_fishery <- function(catch_df){
  ...
  p1 <- catch_df %>%
    filter(Year >= burnin+1) %>%
    filter(!is.na(catch_mt)) %>%
    group_by(Year,run,cap,wgts,env) %>%
    summarise(catch_tot = sum(catch_mt),
              biom_tot = sum(biom_mt_selex)) %>%
    ungroup() %>%
    pivot_longer(-c(Year:env)) %>%
    filter(wgts != "binary") %>%
    ggplot(aes(x = Year+1990, y = value, color = factor(cap, labels = c("800 Kt", "600 Kt", "400 Kt", "200 Kt")), linetype = factor(wgts)))+
  ...
  for(i in seq_along(oy_species)) {
    ...
    p_df <- catch_df %>%
      ...
      pivot_longer(-c(Year, Code, Name, run, cap, wgts, env))
    ...
    p_tmp <- ggplot(data = p_df %>% filter(Year > burnin+1), 
                    aes(x = Year+1990, y = value, color = factor(cap, labels = c("800 Kt", "600 Kt", "400 Kt", "200 Kt")), linetype = factor(wgts))) +
  ...
  for(i in seq_along(oy_species)){
    ...
    p2 <- catch_df %>%
      filter(Year >= burnin + 5) %>% # shave off the first 5 years after the start of the HCR being applied or esle all biomass starts from the same place
      filter(Code == current_code, !is.na(catch_mt)) %>%
      ggplot(aes(x = biom_frac, y = f/fref, color = Year+1990))+
      ...
      labs(x = "B/B0", 
           ...
           title = current_name)+
      facet_grid(env~cap,
                 labeller = labeller(env = as_labeller(c("NoClimate" = "Constant\nclimate"))))
  ...
  #   mutate(decade = ceiling(Year/10)) %>%  # Create decade variable
  #   filter(decade %in% c(4, 11)) %>%  # Filter to decades 4, 11 (beginning and end of run)
  ...
  #   mutate(decade = floor(Year/10)) %>%  # Create decade variable
  #   filter(decade %in% c(4,11)) %>%  # Filter to decades 4, 11 (beginning and end of run)
  #   group_by(decade, run, cap, wgts, env) %>%
  #   summarise(
